...
> **Status — Phase 1 (foundation vertical slice).** Read, visualize, search,
> filter, and sort your real bookmarks end-to-end with no API key required. AI
> analysis, metadata collection, category management, and apply-to-Chrome land
> in later phases — see [ROADMAP.md](./ROADMAP.md).
...
```
src/
├─ background/   MV3 service worker (lifecycle; write/long-jobs land later)
├─ popup/        Toolbar popup: stats + "Open Manager"
├─ options/      Full-page manager app (tree, toolbar, settings)
│  ├─ components/  BookmarkTreeView, Toolbar, SettingsPanel
...
├─ state/        Zustand stores (ui, settings) + pure selectors
├─ ui/           Reusable presentational components
└─ utils/        Query client, chrome.storage adapter
```
...
only in `chrome.storage.local`. When AI analysis ships, sending data to a
provider will always be an explicit, opt-in action with a visible scope
(selected bookmarks / folder / all) — see the Privacy section of the roadmap.
